...
| Reporting / MIS | **B** | **Bounded reporting layer done (Steps 9-13).** Filterable search/list on applications, customers (`+status`), products; `GET /reports/contracts` (paginated/filterable), `GET /reports/profitability` (`level=portfolio/category/product/customer` drill-down), 5 `GET /reports/summary/*` aggregates, ~10 per-category sub-reports + `GET /reports/aging` (DPD buckets + drill-down), CSV / **Excel / PDF** export on every report endpoint; a **read-only** 5-tab Executive Dashboard + a Reports Center with per-category sub-reports. `B`: no charts, no scheduled/emailed/saved reports, no ECL-based metrics; DPD buckets are display-only. |
...
| S-8 | ~~**No reporting surface.**~~ **Addressed (G-22, Steps 9-11).** Review queue, customer/product search, `GET /reports/contracts` + `GET /reports/profitability` + 5 dashboard-summary endpoints + CSV export; a 5-tab Executive Dashboard and Reports Center in the UI. No charts / scheduled reports / ECL-based metrics. | ~~**P1**~~ core done |
...
> **Update (post-assessment P0 work):** the five most-severe findings **S-1 through
> S-5 are now all addressed** — S-1 by P0-2 (referred → manual verification), S-4
> by P0-1 (immutable ledger, dual-write phase), S-2 by P0-3 (affordability
> re-check at offer time), S-3 by P0-4 (company-wide exposure aggregation), and
...
| G-08 | Inventory / fulfillment | **Partially closed — Step 10.** `Product` gained flat `stock_quantity`/`reserved_quantity` (no branch/warehouse dimension) and a working-default deduction point (BDR-18). Delivery confirmation is still a bare status flip. | `models/product.py`, `offers.py:confirm_delivery` | Double-sell of the **last unit** is now prevented; still: no serial/delivery record, no branch/warehouse, no `Reservation`/`Delivery` entities | Minimal: `InventoryItem` (sku, branch/warehouse, qty available, qty reserved), `Reservation` (contract/order, qty, status), `Delivery` (contract, scheduled/completed, `serial_number`/`imei`, confirmed_by). Integration boundary to a real WMS. | **P1** | New (minimal domain) + Integrate | Reservation & deduction point — **BDR-18 (structure done)**; ownership transfer — **BDR-01** |
...
| G-22 | Reporting / list endpoints | **Largely closed — Steps 9-11.** `GET /applications?status=` (S9), `GET /customers?search=` / `GET /products?search=` (S10), and Step 11's `GET /reports/contracts` (paginated/filterable), `GET /reports/profitability`, 5 `GET /reports/summary/*` tab aggregates, CSV export on the directory/case-list endpoints, and a 5-tab Executive Dashboard + Reports Center in the UI. | `api/reports.py`, `services/reports.py`, `frontend/src/pages/{DashboardPage,ReportsPage}.tsx` | Remaining: no charts, no scheduled/emailed/saved reports, no Excel/PDF, no ECL-based portfolio-at-risk (no ECL data) | *(done)* filterable contract list + MIS aggregates + CSV; *(remaining)* charts, exports beyond CSV, scheduled delivery | **P1** | New (read-only, over existing tables) | DPD **display** buckets — `dpd_report_buckets` (display grouping only, **not** a DPD-action policy); real report definitions still light **BDR** |
...
| BDR-18 | Inventory reservation & deduction point (at offer / at contract / at delivery) | Stock accuracy vs over-reservation | **Step 10 done (working default, not confirmed policy).** `Product.stock_quantity` is deducted one unit at **contract creation** (offer acceptance) — chosen only so the platform can't double-sell the last unit; cancellation/return release it back. `available_quantity > 0` is checked at offer generation. Reservation-at-offer and deduction-at-delivery remain equally legitimate and unbuilt. | `services/offers.py`, `services/closure.py`, `models/product.py` | Retail ops + WMS team. |
...
| BDR-26 | Reconciliation matching rules + tolerance + auto vs manual threshold | Recon accuracy | **P0-5 done (structure).** `app/services/reconciliation.py` matches `unreconciled` payments to mock `BankStatementLine` rows: exact `bank_reference` → `external_reference`/`gateway_reference`, then amount + value-date within `reconciliation_date_tolerance_days` (placeholder **0** = same day), else a `ReconciliationException` (`no_match`/`amount_mismatch`/`duplicate_candidate`). Manual match via the generic maker-checker (`reconciliation.manual_match`). Still runs on manually-imported lines — no real bank feed. The tolerance value and any auto-vs-manual threshold beyond "open an exception" are unconfirmed. | `config` (`reconciliation_date_tolerance_days`) + `reconciliation` service | Finance + bank. |
...
| BDR-31 | Chart-of-accounts / debit-credit mapping per accounting `event_type`; real-time vs batched posting | Whether the GL is correct | **Architecture now built** (Chart of Accounts feature — see [docs/chart-of-accounts/BRD.md](chart-of-accounts/BRD.md) / [FSD.md](chart-of-accounts/FSD.md)): a versioned `EventAccountMapping` per event type resolves each `AccountingEvent` into a balanced `GLJournal`. **Every seeded account and mapping is an explicit DEMO / ILLUSTRATIVE PLACEHOLDER** — the real chart of accounts, account codes, and approved debit/credit treatment are still unconfirmed. Posting to the mock GL remains an on-demand job (not a scheduler). | `app/models/gl.py`, `app/services/coa.py`, `app/services/gl_journal.py` | Finance + ERP team. Ties to BDR-15. |
| BDR-32 | DPD action thresholds (when each day-past-due band triggers a call / SMS / field visit / legal / write-off recommendation) | Collections operating model | **Not decided.** Step 11 added `dpd_report_buckets` (`[[1,30],[31,60],[61,90],[91,null]]`) — a **reporting-display grouping only** for the Portfolio dashboard's aging table. It is explicitly not a policy on when collections should act; that is a separate, unmade decision. | `config` (`dpd_report_buckets` today, action thresholds later) + `collections` / `overdue` services | Collections policy owner. Ties to BDR-10, BDR-25. |
