...
| BDR-07 | Exposure aggregation dimensions (company / BU / brand / category) | How total customer risk is measured | **none** | new `config` + `Exposure` read-model | — |
| BDR-08 | Multiple active contract policy (max count, max total outstanding, per-category limits) | Whether a 4th contract is allowed | **none** (unlimited) | new `config`, checked in assessment | — |
...
| BDR-22 | Affordability obligation figure (requested amount / priced installment / installment-sale-price ÷ tenor) + whether to re-assess after pricing | Whether credit decisions are sound | `requested_amount / tenor × factor(1.0)`; no re-check | `config` + assessment/offer services | Credit-policy owner. Ties to BDR-05. |
