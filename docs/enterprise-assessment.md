...
| Reporting / MIS | **B** | **Bounded reporting layer done (Steps 9-13).** Search/list on applications, customers, products; `GET /reports/contracts` (paginated/filterable), `GET /reports/profitability`, 5 `GET /reports/summary/*` aggregates, ~10 per-category sub-reports + `GET /reports/aging` (DPD buckets + drill-down), CSV / **Excel / PDF** export on every report endpoint; 5-tab Executive Dashboard + a Reports Center with per-category sub-reports. `B`: no charts, no scheduled/emailed/saved reports, no ECL-based metrics; DPD buckets are display-only. |
...
| G-22 | Reporting / list endpoints | **Largely closed — Steps 9-13.** `GET /applications?status=` (S9), `GET /customers/products ?search=` (S10), Step 11's contract list / profitability / 5 tab summaries, and Step 13's ~10 per-category sub-reports + `GET /reports/aging` (bucket summary + `?bucket=` drill-down) with **csv/xlsx/pdf** export on every report endpoint; UI = 5-tab Executive Dashboard + a Reports Center with per-category sub-reports. | `api/reports.py`, `services/reports.py`, `frontend/src/pages/{DashboardPage,ReportsPage}.tsx` | Remaining: no charts, no scheduled/emailed/saved reports, no ECL-based portfolio-at-risk (no ECL data) | *(done)* filterable lists + MIS aggregates + sub-reports + aging + csv/xlsx/pdf; *(remaining)* charts, scheduled delivery, saved definitions | **P1** | New (read-only, over existing tables) | DPD **display** buckets — `dpd_report_buckets` (display grouping only, **not** a DPD-action policy — see BDR-32); real report definitions still light **BDR** |
