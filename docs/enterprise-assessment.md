...
| Reporting / MIS | **B** | **Bounded reporting layer done (Steps 9-11).** Search/list on applications, customers, products; `GET /reports/contracts` (paginated/filterable), `GET /reports/profitability`, 5 `GET /reports/summary/*` aggregates; CSV export; 5-tab Executive Dashboard + Reports Center UI. `B`: no charts, no scheduled/emailed/saved reports, no Excel/PDF, no ECL-based metrics; DPD buckets are display-only. |
